(function() {
		...
		$('li').click(function(e) {
			...
			$.ajax({
				...
				.done(function(response) {
					console.log('Resort ID response: ',response);
	...
	$.ajax({
		...
		.done(function(response) {
			console.log('Got List', response); //logs the array
...
//jas compare code 
	...
	$.ajax({
		...
		.done(function(response) {
			console.log('Got response for compare',response); //logs the array
			// resortNames = response;
			// searchField.removeAttribute('disabled'); //enables input field when resorts are loaded
		});
//end of jas compare code 
...
function initialize() {
	...
	var markers = [
		...
		['Beaver Creek', 39.6042, -106.5165, "http://localhost:8080/places/Beaver%20Creek"],
		...
		['Purgatory Resort', 37.6303, -107.8140, "http://localhost:8080/places/Purgatory"],
